 ...
 */ function getValuesFromBundle(bundle, sort = true) {
    const values = [];
    ...
    if (entries) {
        ...
        if (sort) values.sort((a, b)=>a.time < b.time ? -1 : 1);
    }
    return values;
}
/**
...
(0, _fhirclientDefault.default).oauth2.ready().then((client)=>{
    ...
    Promise.all([
        client.patient.request("Observation?code=29463-7").then((weights)=>populateListFromValues("wt_list", weights)),
        client.patient.request("Observation?code=8302-2").then((heights)=>populateListFromValues("ht_list", heights))
    ]).then(([weights, heights])=>{
        const bmiEl = document.getElementById("bmi");
        if (bmiEl && weights.length > 0 && heights.length > 0) bmiEl.innerHTML = calculateBMI({
...
},{"fhirclient":"l96Gb","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["iJYvl","h7u1C"], "h7u1C", "parcelRequire17b7")
